...
func (h *sentPacketHandler) detectLostPackets(now time.Time, encLevel protocol.EncryptionLevel) error {
	...
	return pnSpace.history.Iterate(func(p *Packet) (bool, error) {
		...
		if p.SendTime.Before(lostSendTime) {
			...
			packetLost = true
			if h.logger.Debug() {
				h.logger.Infof("\tSMBStream lost packet %d (reordering threshold)", p.PacketNumber)
			}
			if h.tracer != nil {
...
func (h *sentPacketHandler) SendMode() SendMode {
	...
	if !h.congestion.CanSend(h.bytesInFlight) {
		if h.logger.Debug() {
			h.logger.Infof("SMBStream Congestion limited: bytes in flight %d, window %d", h.bytesInFlight, h.congestion.GetCongestionWindow())
		}
		return SendAck
	}
